  ...
  return (
    <div className="w-[var(--sidebar-w)] flex-shrink-0 bg-[var(--surface-1)] border-r border-[var(--border)] flex flex-col h-screen overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4 scrollbar-thin">
        <div className="px-2 pt-3.5 pb-1.5 text-[10px] font-semibold uppercase tracking-[1.2px] text-[var(--text-dim)]">
          Meetings
        </div>
        ...
        {meetings.map((m) => (
          <div
            ...
            className={`flex items-center gap-2.5 py-2.5 px-2.5 rounded-[var(--radius-sm)] cursor-pointer transition-colors mb-0.5 ${
              currentScreen === 'meeting' && currentMeetingId === m._id
                ? 'bg-[var(--surface-3)]'
        ...
        {conversations.map((c) => (
          <div key={c._id}>
            <div
              ...
              className={`py-2.5 px-2.5 rounded-[var(--radius-sm)] cursor-pointer transition-colors mb-0.5 ${
                currentScreen === 'setup' && currentConversationId === c._id
                  ? 'bg-[var(--surface-3)]'
                  : 'hover:bg-[var(--surface-2)]'
              }`}
            >
                    ...
                    >
                      <div
                        className={`w-[5px] h-[5px] rounded-full flex-shrink-0 ${
                          m.status === 'active' ? 'bg-[var(--accent)]' : 'bg-[var(--amber)]'
                        }`}
                      />
      ...
      <div className="px-2 py-2 pb-3 border-t border-[var(--border)] flex-shrink-0">
        {(['history', 'settings'] as Screen[]).map((s) => (
          <div
            key={s}
            onClick={() => setScreen(s)}
            className={`flex items-center gap-2 py-[7px] px-2.5 rounded-[var(--radius-sm)] cursor-pointer text-[12.5px] text-[var(--text-muted)] transition-all hover:bg-[var(--surface-2)] hover:text-[var(--text)] ${
              currentScreen === s ? 'bg-[var(--surface-3)] text-[var(--text)]' : ''
            }`}
          >
