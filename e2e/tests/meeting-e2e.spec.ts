 ...
 *   pnpm test:meeting
 *
 * Requires: Docker Compose stack running (mongodb, redis, server)
 * Set MEETING_E2E=true to enable these tests.
 */
...
/** Collect all SSE events of a given type from a response body. */
function parseSSEEvents(body: string, type: string): Record<string, unknown>[] {
  const results: Record<string, unknown>[] = [];
  for (const line of body.split('\n')) {
    if (!line.startsWith('data: ')) continue;
    try {
      const parsed = JSON.parse(line.slice(6)) as Record<string, unknown>;
      if (parsed.type === type) results.push(parsed);
    } catch {
      // skip
    }
  }
...
test.describe('Full Meeting E2E Flow', () => {
  ...
  test.beforeEach(() => {
    if (!process.env.MEETING_E2E) {
      test.skip(true, 'Set MEETING_E2E=true to run full meeting e2e tests');
    }
  });
  ...
  test('3. Send topic message → MindX returns agent suggestions', async ({ request }) => {
    ...
    const res = await request.post(`/api/conversations/${conversationId}/message`, {
      data: { content: 'Tôi muốn thảo luận chiến lược định giá (pricing strategy) cho một SaaS B2B tại thị trường Việt Nam' },
    });
    ...
    expect(agentsMsg, 'MindX should return bot-agents for a clear SaaS pricing topic').toBeDefined();
    ...
    // Also verify progressive agent_suggested events were emitted during stream
    const progressiveEvents = parseSSEEvents(body, 'agents_suggested');
    expect(progressiveEvents.length, 'Progressive agents_suggested events should be emitted').toBeGreaterThan(0);
  ...
  test('5. Meeting runs to completion — agent turns + wrapup', async ({ request }) => {
    ...
    const POLL_INTERVAL_MS = 20_000; // poll every 20 s
    ...
    while (Date.now() - startMs < MAX_WAIT_MS) {
      ...
      const msgCount = (poolData.messages as unknown[]).length;

      if (status !== lastStatus || msgCount !== lastMsgCount) {
        console.log(`[Meeting] status=${status} messages=${msgCount} elapsed=${Math.round((Date.now() - startMs) / 1000)}s`);
        lastStatus = status;
  ...
  test('6. Verify final pool state and message integrity', async ({ request }) => {
    ...
    // All agent turn messages should have non-empty content
    for (const msg of messages) {
      if (msg.agentId !== 'user') {
        expect(
    ...
    const speakingAgents = (pool.agents as Array<Record<string, string>>).filter(
      a => a.state === 'speaking' || a.state === 'queued',
    );
    expect(speakingAgents.length, 'No agents should be speaking/queued after completion').toBe(0);
  });
});
